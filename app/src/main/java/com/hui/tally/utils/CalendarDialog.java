...
public class CalendarDialog extends Dialog implements View.OnClickListener {
    ...
    private void addViewToLayout() {
        hsvViewList = new ArrayList<>();   //A collection of TextViews that will be added into the linear layout for unified management
        yearList = DBManager.getYearListFromAccounttb(); //Get how many years are stored in the database
        //If there is no record in the database, add this year's record
        if (yearList.size() == 0) {
            int year = Calendar.getInstance().get(Calendar.YEAR);
            yearList.add(year);
        }

        //Traversing the year, if there are several years, add a few views to the ScrollView
